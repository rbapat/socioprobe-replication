...
if __name__ == "__main__":
    ...
    for idx, score_type in enumerate(("F1", "MDL")):
        ...
        plot_data(cola_electra_f1_scores_layerall[idx], "ELECTRA", score_type, "Acceptability", data_name)
        plt.savefig(path)
        plt.figure()
        print(f"Saved {path}")
    ...
    for idx, score_type in enumerate(("F1", "MDL")):
        path = os.path.join("figures", f"{model_type}_{score_type}.png")
        plot_data(cola_robertabase_f1_scores_layerall[idx], "RoBERTa-base", score_type, "Acceptability", data_name)
        plt.savefig(path)
        plt.figure()
        print(f"Saved {path}")
