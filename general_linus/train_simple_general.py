...
from probes_general import SimpleProbe
...
def run_epoch(
    ...
    # Change how final F1 is calculated based on # of classes
    average = "binary" if orig_dataset.get_output_dim() == 2 else "weighted"

    return running_loss / len(running_preds), metrics.f1_score(
        running_gt, running_preds, average=average
    )
...
def train_probe(
    ...
    probe = SimpleProbe(hidden_dim, out_dim).to(device)
    ...
    test_loss, test_f1 = run_epoch(probe, optimizer, loss_function, test_loader, False, is_test=True)
    if verbose:
