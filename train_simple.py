...
def run_epoch(model: torch.nn.Module,
              ...
              is_train: bool,
              probe_age: bool,
              probe_gender: bool) -> Tuple[float, float]:
    ...
    Args:
        ...
        probe_age (bool): If true, the ground truth for training is age and `probe_gender` must be false
        probe_gender (bool): If true, the ground truth for training is gender and `probe_age` must be false
    ...
    for embeds, gender, age in dataloader:
        optimizer.zero_grad()
...
def train_probe(train_set: TrustPilotDataset,
                val_set: TrustPilotDataset,
                test_set: TrustPilotDataset,
                device: torch.device,
                probe_age: bool = False,
                probe_gender: bool = False,
    ...
    Args:
        train_set (TrustPilotDataset): training dataset
        val_set (TrustPilotDataset): validation dataset
        test_set (TrustPilotDataset): testing dataset
    ...
    while no_improvement < es_patience:
        train_loss, train_f1 = run_epoch(model, optimizer, loss_function,
                                         train_loader, True, probe_age, probe_gender)
        val_loss, val_f1 = run_epoch(model, optimizer, loss_function,
                                     val_loader, False, probe_age, probe_gender)
    ...
    test_loss, test_f1 = run_epoch(model, optimizer, loss_function,
                                   test_loader, False, probe_age, probe_gender)
    if verbose:
