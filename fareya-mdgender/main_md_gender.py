...
if __name__ == "__main__":
    ...
    data_location = "md_gender_20k.csv"
    data_name = "mdgender"
    ...
    for idx, score_type in enumerate(("F1", "MDL")):
        ...
        plot_data(cola_electra_f1_scores_layerall[idx], "ELECTRA", score_type, "Gender", data_name)
        plt.savefig(path)
        plt.figure()
        print(f"Saved {path}")
    ...
    for idx, score_type in enumerate(("F1", "MDL")):
        path = os.path.join("figures", f"{model_type}_{score_type}.png")
        plot_data(cola_robertabase_f1_scores_layerall[idx], "RoBERTa-base", score_type, "Gender", data_name)
        plt.savefig(path)
        plt.figure()
        print(f"Saved {path}")
    ...
    for idx, score_type in enumerate(("F1", "MDL")):
        path = os.path.join("figures", f"{model_type}_{score_type}.png")
        plot_data(cola_robertabase_f1_scores_layerall[idx], "DeBERTa-base", score_type, "Gender", data_name)
        plt.savefig(path)
        plt.figure()
        print(f"Saved {path}")
    ...
    # Output layer (layer 0)
    model_type = "microsoft/deberta-base"
    cola_debertabase_f1_scores_layer0 = model_pipeline(model_type, data_location, verbose=verbose, layer=12)
